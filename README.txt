...
- Cost constant time * l, where l is the length of the prefix(cost of a search is basically constant)
- Easy to add new words
- Easy to remove words
For those reasons I think it solves the detailed problem nicely.

My DFA structure basically consists of States, where each node has a HashMap<char,State>. My reasoning with this is to have pretty much constant time on all cases(except the exponentially uncommon). Since the access time is generally constant in access time for hashmaps, and traversing each step is constant. 
...
With the current implementation, you could also add another function to get the amount of responses given a prefix, as I think it would be valuable function. 
...
In order to perform searches like our program does, we would want to use the "keyword" specification for each word we index. If we want to have the data a bit more partitioned, I would recommend using a different index for each language. Even better than that would be one for each physical region to avoid concept drifting/local differences. To give an example I know about, the word "carro" in Spain would never be used to refer to a car, but in Mexico you would. Locality is the the way to keep it simple in natural language usually.

Other things to consider would be misstypes, since a good autocomplete should be tolerant ot those. ElasticSearch allows for that in text searches(with very few character transformations on fuzzy queries). You may want to use suggest fields to allow more transformations. If you can determine different searches are equal and you want to unite them that would be an improvement too and you could have them be the response of similar suggests.
